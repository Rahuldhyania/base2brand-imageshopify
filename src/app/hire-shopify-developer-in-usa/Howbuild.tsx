import React from "react";

type howbuild = {
  title: string;
  description: string;
};
const Howbuild = ({ title, description }: howbuild) => {
  return (
    <div className="b2b-black-bg">
      <div className="cus_container max_w_1920 py-5">
        <div className="text-center">
          <p className="text_hire_primary pb-3  pb-lg-0 b2b-title-text fw-bold">
            Our Proven Process
          </p>
          <h2
            className="hire_banner_subheading text-white"
            dangerouslySetInnerHTML={{ __html: title }}
          />
          <p
            className="b2b-title-text text-white"
            dangerouslySetInnerHTML={{ __html: description }}
          />
        </div>
        <div className="py-3 build_main_continer">
          <div className="buid_icon_container">
            <div className="buid_icon_outer">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="48"
                height="48"
                viewBox="0 0 24 24"
              >
                <rect width="24" height="24" fill="none" />
                <path
                  fill="#fff"
                  d="m13.12 17.023l-4.199-2.29a4 4 0 1 1 0-5.465l4.2-2.29a4 4 0 1 1 .958 1.755l-4.2 2.29a4 4 0 0 1 0 1.954l4.2 2.29a4 4 0 1 1-.959 1.755M6 14a2 2 0 1 0 0-4a2 2 0 0 0 0 4m11-6a2 2 0 1 0 0-4a2 2 0 0 0 0 4m0 12a2 2 0 1 0 0-4a2 2 0 0 0 0 4"
                />
              </svg>
              <p className="build_icon_count">1</p>
            </div>
            <div className="buid_icon_outer">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="48"
                height="48"
                viewBox="0 0 24 24"
              >
                <rect width="24" height="24" fill="none" />
                <g fill="#fff" fillRule="evenodd" clipRule="evenodd">
                  <path d="M23.627 7.51c-.13-1.17-.07-3.511-.91-4.151c-1.922-1.491-6.133-1.091-8.815-1.201c-3.501-.16-7.153.14-10.614.24a.34.34 0 0 0 0 .67c3.461-.07 7.073-.28 10.574-.07c2.471.14 6.673-.13 8.254 1.13c.4.321.41 2.682.49 3.492c.337 2.917.364 5.862.08 8.784a13.3 13.3 0 0 1-.45 2.891c-2.381 1.331-19.659 2.361-20.71.54a20.8 20.8 0 0 1-.64-4.521a39.6 39.6 0 0 1 .15-7.684c.08 0 7.304.33 12.576.23c2.331 0 4.672-.16 6.763-.2a.34.34 0 1 0 0-.68C12.492 6.82 7.62 6.55 1.077 7c0-.55.1-1.06.17-1.61a7.8 7.8 0 0 1 .21-1.891a.3.3 0 0 0-.42-.42c-.42.44-.34 1.78-.41 2.25S.487 6.38.427 6.9a43.4 43.4 0 0 0-.36 8.464c.023 1.658.248 3.308.67 4.912c1.46 2.671 17.247 1.44 20.929.44c1.45-.39 1.69-.79 1.88-1.92a38.8 38.8 0 0 0 .08-11.286" />
                  <path d="M4.528 5.389a.81.81 0 0 0-.21-1c-.76-.4-1.7.38-1 1.08a.84.84 0 0 0 1.21-.08m2.631-.28a.8.8 0 0 0-.2-1c-.77-.41-1.71.38-1 1.07a.84.84 0 0 0 1.2-.07m2.881 0a.8.8 0 0 0-.21-1c-.77-.41-1.71.38-1 1.07a.84.84 0 0 0 1.21-.07M3.458 18.005a2.26 2.26 0 0 0 2-.63c.62-.82.35-2.461-1.64-2.411a1.55 1.55 0 0 0-.36 3.04m1.45-1.36a1.07 1.07 0 0 1-1.06.29c-.37-.13-.5-.72-.33-.881c.63-.64 1.77-.27 1.39.59m5.082-5.021a.3.3 0 0 0 .31-.51c-1.49-1-4.001 1.72-3.3 3.922c.75 2.38 3.35 2.47 4.001 1.1c.21-.44.18-1.07-.3-1a.34.34 0 0 0-.34.33c0 .44-.07.69-.68.71c-.77 0-1.321-.6-1.571-1.43c-.44-1.331.88-3.622 1.88-3.122m2.002.48c-.88 2.111-.73 4.802 1.56 4.872a1.48 1.48 0 0 0 1.55-1c.271-.71.841-4.482-1.23-4.932a1.63 1.63 0 0 0-1.88 1.06m2.36 3.192c-.1.38-.29.66-.75.62c-1.31-.09-1.08-2.331-.69-3.471a.77.77 0 0 1 .82-.61c1.11.17.78 2.6.57 3.46zm2.642-2.201c.28.38.59.81.78 1a.8.8 0 0 0 .81.3c.71-.28.77-1.53 1.06-2.19a16.6 16.6 0 0 1 .521 3.65c0 .291 0 .741.43.581c.34 0 .28-.32.27-.62c-.07-3.732.06-4.412-.94-4.772c-.31-.12-.73 0-1.05.54q-.29.724-.48 1.48c-.36-.43-.861-1.09-1.141-1.35c-.49-.48-1-.3-1.07.12c-.38 1.69.29 3.762.37 4.662a.3.3 0 1 0 .59 0a19 19 0 0 1-.15-3.401" />
                </g>
              </svg>
              <p className="build_icon_count">2</p>
            </div>
            <div className="buid_icon_outer">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="48"
                height="48"
                viewBox="0 0 24 24"
              >
                <rect width="24" height="24" fill="none" />
                <g
                  fill="none"
                  stroke="#fff"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth="1.4"
                >
                  <path d="M6.677 20.567C2.531 18.021.758 12.758 2.717 8.144C4.875 3.06 10.745.688 15.829 2.846s7.456 8.029 5.298 13.113a9.95 9.95 0 0 1-3.962 4.608" />
                  <path d="M17 16v4.4a.6.6 0 0 0 .6.6H22m-10 1.01l.01-.011" />
                </g>
              </svg>
              <p className="build_icon_count">3</p>
            </div>
            <p className="build_hr_line">
              <span />
            </p>
          </div>
          <div className="buid_text_container pt-4">
            <div className="text-center build_text_outer">
              <div>
                <h2 className="b2b-partner-heading m-0 text_hire_primary pt-2">
                  Strategy First
                </h2>
                <p className="b2b-text text-white">
                  We begin by understanding your brand, audience, and goals.
                  Every decision is backed by marketing insight, not guesswork.
                </p>
              </div>
            </div>
            <div className="text-center build_text_outer">
              <h2 className="b2b-partner-heading m-0 text_hire_primary pt-2">
                Smart Design & Development
              </h2>
              <p className="b2b-text text-white">
                Our team builds user-friendly, mobile-optimized, and SEO-ready
                stores — built to support your digital marketing success.
              </p>
            </div>
            <div className="text-center build_text_outer">
              <h2 className="b2b-partner-heading m-0 text_hire_primary pt-2">
                Launch & Growth
              </h2>
              <p className="b2b-text text-white">
                After launch, we connect your store with campaigns like SEO,
                ads, and email to scale traffic and revenue.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Howbuild;
